...
class LSSerializer(DOMObject):
  ...
  def write(self, node, destination):
    ...
    node._writeTo(
      buffer, self._domConfig, self._filter, self._newLine, namespaces, depth=0
    )
    return buffer.flush()
...
def _Node___writeTo(self, dest, config, filter, newLine, namespaces, depth):
  """ Markup production, for various node types. The default node behaviour is
      just to recurse to all children.
  """
  depth=depth+1
  for child in self._childNodes:
    child._writeTo(dest, config, filter, newLine, namespaces, depth)


def _Document___writeTo(self,dest,config,filter,newLine,namespaces, depth):
  newLine = os.linesep
...
def _Element___writeTo(self, dest, config, filter, newLine, namespaces, depth):
  ...
  for attr in attrs:
    ...
    if first_attr_newline:
        dest.setSeparator(newLine + (depth + 1) * '    ')
    else:
        dest.setSeparator(' ')
    ...
    if self._childNodes.length!=0:
      ...
      if not config.getParameter('format-pretty-print') or (
        ...
        NamedNodeNS._writeTo(
          self, dest, config, filter, newLine, newspaces, depth
        )
      else:
        dest.write(newLine+'  ')
        NamedNodeNS._writeTo(
...
def _Attr___writeTo(
  self, dest, config, filter, newLine, namespaces, prefix= NONS, depth = 0
):
...
def _Comment___writeTo(self,dest,config,filter,newLine,namespaces, depth):
...
def _Text___writeTo(
  self, dest, config, filter, newLine, namespaces, attr= False, depth = 0
):
  if (
...
def _CDATASection___writeTo(
  self, dest, config, filter, newLine, namespaces, depth
):
...
def _EntityReference___writeTo(self,
  dest, config, filter, newLine, namespaces, attr= False, depth = 0
):
